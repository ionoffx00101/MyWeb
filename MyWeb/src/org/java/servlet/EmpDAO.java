...
public class EmpDAO extends AbstractDAO {
	...
	public boolean memberjoin(MemverVO mv) {
		conn = getConn();
		String sql = "insert into member(id,pwd,gender,birth,persnal) values(?,?,?,?,?)";
		...
		try {
			...
			pstmt.setString(3, mv.getGender());
			pstmt.setDate(4, mv.getBirth());
			pstmt.setString(5, mv.getPersnal());
			pstmt.executeUpdate();
